...
import miditoolkit_ped.miditoolkit as miditoolkit
from miditoolkit_ped.miditoolkit.midi.containers import Marker, Instrument, TempoChange, Note, Pedal
...
###--- data ---###
path_data_root = './'
path_train_data = os.path.join(path_data_root, 'ailabs_train_data_linear.npz')
path_dictionary =  os.path.join(path_data_root, 'dictionary_mod.pkl')

###--- training config ---###
D_MODEL = 512
N_LAYER = 12
N_HEAD = 8    
path_exp = 'exp_pedal/0706-1337'
batch_size = 4
gid = 1
init_lr = 0.0001
...
path_gendir = './exp_pedal/0706-1337/gen_midis/loss_22'
num_songs = 5
        ...
        if vals[4] == 'Metrical':
            if vals[3] == 'Bar':
                ...
                if vals[2] != 'CONTI' and vals[2] != 0:
                    if vals[2] != 'N_N':
                        duration = int(vals[2].split('_')[-1])
                        #midi_obj.instruments.pedals.append(Pedal(start=start, end=end, duration=end-start))
                        all_pedals.append(Pedal(start=cur_pos, end=cur_pos+duration))
...
class TransformerModel(nn.Module):
    def __init__(self, n_token, is_training=True):
        ...
        self.emb_sizes = [128, 256, 128, 64, 32, 512, 128, 128]
        ...
        self.word_emb_pedal     = Embeddings(self.n_token[2], self.emb_sizes[2])
        ...
        self.proj_pedal    = nn.Linear(self.d_model, self.n_token[2])
    ...
    def train_step(self, x, target, loss_mask):
        ...
        y_tempo, y_chord, y_pedal, y_barbeat, y_pitch, y_duration, y_velocity = self.forward_output(h, target)
         
        # reshape (b, s, f) -> (b, f, s)
        y_tempo     = y_tempo[:, ...].permute(0, 2, 1)
        y_chord     = y_chord[:, ...].permute(0, 2, 1)
        y_pedal     = y_pedal[:, ...].permute(0, 2, 1)
        ...
        loss_pedal = self.compute_loss(
                y_pedal, target[..., 2], loss_mask)
        ...
        return loss_tempo, loss_chord, loss_pedal, loss_barbeat, loss_type, loss_pitch, loss_duration, loss_velocity
    ...
    def forward_hidden(self, x, memory=None, is_training=True):
        ...
        emb_pedal =    self.word_emb_pedal(x[..., 2])
    ...
    def froward_output_sampling(self, h, y_type):
        ...
        cur_word_pedal =    sampling(y_pedal, t=2, p=0.9)
