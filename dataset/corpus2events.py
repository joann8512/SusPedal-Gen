...
def create_piano_metrical_event(tempo, chord, pedal, pos):
    meter_event = compound_event.copy()
    meter_event['tempo'] = tempo
    meter_event['chord'] = chord
    meter_event['bar-beat'] = pos
    meter_event['pedal'] = pedal
    meter_event['type'] = 'Metrical'
    return meter_event
...
def corpus2event_cp(path_infile, path_outfile):
    ...
    for bar_step in range(0, global_end, BAR_RESOL):
        ...
        for timing in range(bar_step, bar_step + BAR_RESOL, TICK_RESOL):
            ...
            t_pedals = data['pedals'][0][timing] # piano track

            # metrical
            if len(t_tempos) or len(t_chords) or len(t_pedals):
                # pedal
                if len(t_pedals):
                    #pedal_text = 'Pedal_' + str(t_pedals[-1].end-t_pedals[-1].start)
                    pedal_text = 'Pedal_' + str(t_pedals[-1].duration)
                    end = t_pedals[-1].end  # reset ending time for each new pedal
                    flag = True  # pedal happening
                else:
                    if not flag and end == -1:  # not yet have first pedal
                        pedal_text = 'N_N'
                        end = timing
                    elif flag and timing < end: # while before ending of pedal
                        pedal_text = 'CONTI'
                    elif not flag and timing > end and end != -1:
                        pedal_text = 'CONTI'
                    elif flag and timing > end:
                        pedal_text = 'N_N'
                        flag = False
                ...
                pos_events.append(
                    create_piano_metrical_event(
                        tempo_text, chord_text, pedal_text, pos_text))
                pos_on = True
...
if __name__ == '__main__':
    # paths
    path_root = './'
    path_indir = os.path.join(path_root, 'MAESTRO_corpus/') #'../corpus/fixed/'
    path_outdir = os.path.join(path_root, 'MAESTRO_events') #, 'fixed')
