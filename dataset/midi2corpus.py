...
PEDAL_DURATIONS = [240, 480, 720, 960, 1440, 1920, 2400, 2880, 3360, 3840]
...
def proc_one(path_midi, path_outfile):
    ...
    # load pedals #
    instr_pedals = collections.defaultdict(list)
    for instr in midi_obj.instruments:
        instr_idx = INSTR_NAME_MAP[instr.name]
        for pedals in instr.pedals:
            instr_pedals[instr_idx].append(pedals)

        instr_pedals[instr_idx].sort(key=lambda x: x.start)
    ...
    # process pedals
    instr_pedal_grid = dict()
    for key in instr_pedals.keys():
        pedals = instr_pedals[key]
        pedal_grid = collections.defaultdict(list)
        for pedal in pedals:
            if pedal.end < first_note_time:
                continue
            pedal.start = pedal.start - offset * BAR_RESOL
            pedal.end = pedal.start + pedal.duration
            quant_time = int(np.round(pedal.start / TICK_RESOL) * TICK_RESOL)

            # shift of start
            pedal.shift = pedal.start - quant_time
            pedal.shift = DEFAULT_SHIFT_BINS[np.argmin(abs(DEFAULT_SHIFT_BINS-pedal.shift))]

            # duration
            pedal_duration = pedal.duration
            if round(pedal_duration/BAR_RESOL) > 4:
                pedal_duration = BAR_RESOL * 4
            quant_dur = 3840
            loss = 4000
            for i in PEDAL_DURATIONS:
                if abs(pedal_duration - i) < loss:
                    #print('loss: ', abs(pedal_duration - i))
                    loss = pedal_duration - i
                    quant_dur = i
            
            # quant_dur
            ptick_duration = int(np.round(quant_dur / TICK_RESOL) * TICK_RESOL)
            pedal.duration = ptick_duration
            #print('dur: ', pedal.duration)
            # append
            pedal_grid[quant_time].append(pedal)

        # set to track
        instr_pedal_grid[key] = pedal_grid.copy()
...
if __name__ == '__main__':
     # paths
    path_indir = './MAESTRO_analyzed'
    path_outdir = './MAESTRO_corpus'
