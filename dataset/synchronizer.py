...
from miditoolkit_ped.miditoolkit.midi import parser
from miditoolkit_ped.miditoolkit.midi.containers import TimeSignature, TempoChange
...
if __name__ == '__main__':
    # paths
    path_audiodir = '../maestro-v2.0.0'
    path_indir = '../adjusted_maestro_pedal'
    path_outdir = './MAESTRO_synchronized_test'
    ...
    for fidx in range(n_files): 
        fn = midifiles[fidx]
        #audio_fn = fn.replace('_', ' ')
        print('{}/{}'.format(fidx, n_files))
        # paths
        path_midi_input = os.path.join(path_indir, fn+'.midi')
        path_midi_output = os.path.join(path_outdir, fn+'.mid')
        path_audio_input = os.path.join(path_audiodir , fn+'.wav')
